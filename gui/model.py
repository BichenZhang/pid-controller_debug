...
class Model:
    ...
    def init_param_map(self):
        ...
        self.pmap = {
                ...
                params.pid_lock_en_addr : [0] * self.n_out,
                params.chan_src_sel_addr : [params.chan_src_sel_init] * self.n_out,
                # Oversample filter
                params.ovr_os_addr : [params.ovr_os_init] * self.n_out,
                #Rep Rate Multiplier
                params.rep_rate_mult_addr : [params.rep_rate_mult_init] * self.n_out,
#                #Reference DDS
#                params.ref_dds_addr : [params.ref_dds_sel_init] * self.n_out,
                # PID filter
                ...
                params.pid_ignore_addr : [params.pid_ignore_init] * self.n_out,
                # Output filter
    ...
    def output_to_string(self, output):
        ...
        if not self.is_valid_output(output):
            ...
            return 'FREQ ' + str(output - freq0)
        elif output < self.params.amp0_addr:
            return 'PHASE ' + str(output - phase0)
        else:
            return 'AMP ' + str(output - amp0)
    ...
    def get_freq_list(self,ntype):
        flist = []
        for chan in range(self.params.n_dac, self.params.n_dac+4):
            flist.append(self.chan_to_string(chan, ntype))
