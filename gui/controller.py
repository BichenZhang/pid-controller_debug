...
class Controller():
    ...
    def register_view_handlers(self):
        ...
        for chan in range(self.params.n_pid_chan):
            self.register_chan_view_handlers(chan, self.params)

    '''
    Register channel view handlers
    '''
    def register_chan_view_handlers(self, chan, params):
        gp_view = self.view.gp_view
...
#        if (chan >= params.freq0_addr) and (chan < params.phase0_addr):
#            config_view.ref_dds_sel.activated.connect(
#                lambda: self.update_ref_dds_sel(chan, config_view.ref_dds_sel.currentIndex()))
        if (chan >= params.freq0_addr) and (chan < params.phase0_addr):
            config_view.rep_rate_mult.valueChanged.connect(
                    lambda: self.update_rep_rate_mult(chan, config_view.rep_rate_mult.value()))
        ...
        if (chan >= params.freq0_addr) and (chan < params.phase0_addr):
            pid_view.pid_ignore.stateChanged.connect(
                lambda: self.update_pid_ignore(chan, pid_view.pid_ignore.isChecked()))
        ...
        if (chan >= params.freq0_addr) and (chan < params.phase0_addr):
        #Put DDS on / off handlers here
            output_view.opt_sin.stateChanged.connect(
                lambda: self.request_opt_sin(chan,output_view.opt_sin.isChecked()))
            output_view.opt_dig.stateChanged.connect(
                lambda: self.request_opt_dig(chan,output_view.opt_dig.isChecked()))
    ...
    def set_view_ranges(self):
        ...
        self.view.gp_view.sample_rate.setRange(1, 200) #Changed from the max of 100
        ...
        for chan in range(self.params.n_pid_chan):
            ...
            error_view.ovr_os.addItems(
                    [str(x) for x in range(2**self.params.w_pid_os)]) #Changed to 2^N #[str(2**x) for x in range(2**self.params.w_pid_os)])
...
#            #DDS Reference
#            if (chan >= self.params.freq0_addr) and (chan < self.params.phase0_addr):
#                config_view.ref_dds_sel.clear()
#                config_view.ref_dds_sel.addItems(['None'] + self.model.get_freq_list('usr'))
            ...
            #Rep Rate Multiplier
            if (chan >= self.params.freq0_addr) and (chan < self.params.phase0_addr):
                config_view.rep_rate_mult.setRange(0,500)
    ...
    def initialize_chan(self, chan, model):
        ...
        # Rep rate multiplier
        if (chan >= params.freq0_addr) and (chan < params.phase0_addr):
            rep_rate_mult = model.get_param(params.rep_rate_mult_addr, chan)
            config_view.rep_rate_mult.setValue(rep_rate_mult)
            self.update_rep_rate_mult(chan, rep_rate_mult)
            
#         # Reference DDS
#        if (chan >= params.freq0_addr) and (chan < params.phase0_addr):
#            ref_dds_sel = model.get_param(params.ref_dds_addr, chan)
#            opt_add_port_dds = self.io_config.map_chan_to_port(ref_dds_sel)
#            config_view.ref_dds_sel.setCurrentIndex(opt_add_port_dds + 1)
#            self.update_ref_dds_sel(chan, ref_dds_sel)
        
        #Ignore PID
        if (chan >= params.freq0_addr) and (chan < params.phase0_addr):
            pid_ignore = model.get_param(params.pid_ignore_addr, chan)
            pid_view.pid_ignore.setChecked(pid_ignore)
            self.update_pid_ignore(chan, pid_ignore)
    ...
    '''
    Rep rate multiplier param update handling
    '''
    def update_rep_rate_mult(self,chan,value):
        self.update_model_and_fpga(self.params.rep_rate_mult_addr, chan, value)
        self.update_scale_factor(chan)
        self.chan_print(chan, "rep rate mult set to " + str(value))
    ...
    '''
    Reference DDS param update handling
    '''
#    def update_ref_dds_sel(self,chan,value):
#        self.update_model_and_fpga(self.params.ref_dds_addr, chan, value)
#        self.chan_print(chan, "Reference DDS set to " + str(value))
    ...
    def update_pid_ignore(self,chan,invert):
        self.update_model_and_fpga(self.params.pid_ignore_addr, chan, invert)
        self.chan_print(chan, "PID ignore " + ("enabled" if invert else "disabled"))
    ...
    def request_opt_sin(self, chan, toggle_value):
        if toggle_value:
            self.send_fpga_request(self.params.opt_sin_ena_rqst, chan);
            self.chan_print(chan, "sine enable sent");
        else:
            self.send_fpga_request(self.params.opt_sin_dis_rqst, chan);
            self.chan_print(chan, "sine disable sent");
        
    def request_opt_dig(self, chan, toggle_value):
        if toggle_value:
            self.send_fpga_request(self.params.opt_dig_ena_rqst, chan);
            self.chan_print(chan, "digital enable sent");
        else:
            self.send_fpga_request(self.params.opt_dig_dis_rqst, chan);
            self.chan_print(chan, "digital disable sent");
